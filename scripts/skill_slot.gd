...
signal tooltip_requested(text: String, pos: Vector2)
...
# 每幀更新冷卻進度，不重建整個 slot
func update_cooldown(cd: float, max_cd: float) -> void:
	if absf(cd - cooldown) > 0.05 or max_cd != max_cooldown:
		cooldown = cd
		max_cooldown = max_cd
		queue_redraw()
...
func _on_mouse_entered() -> void:
	var text := description if description != "" else "%s LV%d" % [skill_name, level]
	tooltip_requested.emit(text, get_global_mouse_position())
