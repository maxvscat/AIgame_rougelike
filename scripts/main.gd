...
func _process_skill_cooldowns(delta: float) -> void:
	...
	# 清一色自動觸發（CD 5秒，不受攻速影響；場上沒有敵人時不發動——新關卡要等敵人生成）
	if flush_cooldown <= 0.0 and player != null and player.get_skill_level("mahjong_flush") > 0 and game_started and not is_game_ended and _enemies_present():
...
func _show_boss_skill_banner(text: String) -> void:
	...
	banner.process_mode = Node.PROCESS_MODE_PAUSABLE
	banner.z_index = 60
	if _game_font != null:
		banner.add_theme_font_override("font", _game_font)
	banner.add_theme_font_size_override("font_size", 40)
	banner.add_theme_color_override("font_color", Color(1.0, 0.35, 0.25))
	banner.add_theme_color_override("font_outline_color", Color(0.0, 0.0, 0.0, 0.95))
	banner.add_theme_constant_override("outline_size", 9)
	banner.scale = Vector2.ONE * 0.6
	banner.pivot_offset = Vector2(get_viewport_rect().size.x * 0.5, 24.0)
	ui_canvas.add_child(banner)
	var tween := create_tween()
	tween.set_pause_mode(Tween.TWEEN_PAUSE_STOP)
...
func _process_moon(delta: float) -> void:
	...
	if moon_cooldown <= 0.0 and moon_volley_phase == "idle" and _enemies_present():
		# 場上沒有敵人時不發動（新關卡要等敵人生成）
		var cd: float = float([0, 4.0, 3.5, 3.0, 2.5, 2.0, 1.5][level])
		moon_cooldown = cd
		skill_cooldowns["mahjong_moon"] = cd   # 技能欄冷卻顯示
...
func _enemies_present() -> bool:
	# 場上是否有存活敵人（攻擊型技能在新關卡要等敵人生成後才發動）
	for e in get_tree().get_nodes_in_group("enemies"):
...
func _get_skill_max_cooldown(skill_id: String) -> float:
	if skill_id.begins_with("fish_"):
		return 15.0
	var lv: int = player.get_skill_level(skill_id) if (player != null and is_instance_valid(player)) else 1
	match skill_id:
		"tech_meltdown": return 2.0
		"tech_magnet": return 5.0
		"mahjong_flush": return 5.0
		"mahjong_wall": return 10.0
		"mahjong_moon": return float([0, 4.0, 3.5, 3.0, 2.5, 2.0, 1.5][clampi(lv, 0, 6)])
		"mahjong_sanyuan": return 5.0
		"dice_execute": return _skill_level_value("dice_execute", "cooldown_s", [0, 6, 5, 4, 3, 2, 1], maxi(lv, 1))
	return 0.0
