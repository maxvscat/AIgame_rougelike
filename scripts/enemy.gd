...
# ── Boss 拉霸機技能 ──────────────────────────────────────────────────────
var _slot_active := false        # 開獎～攻擊發動期間：Boss 停止移動與普攻、受傷 +25%
var _boss_skill_toggle := false  # 與雷射技能交錯使用
var _enrage_timer := 0.0         # 狂暴（骷髏符號）剩餘時間
...
func _process_status(delta: float) -> void:
	if _enrage_timer > 0.0:
		_enrage_timer -= delta
		if _enrage_timer <= 0.0 and _enrage_applied:
			_enrage_applied = false
			move_speed /= 1.4
...
func take_damage(amount: float) -> float:
	...
	if _slot_active:
		final_damage *= 1.25   # 開獎期間受傷 +25%（獎勵貼臉輸出）
...
func _start_boss_slot() -> void:
	if _slot_active or not is_instance_valid(target):
		return
	_slot_active = true
	var slot_script := load("res://AIgame_rougelike/scripts/boss_slot_machine.gd")
	if slot_script == null:
		_slot_active = false
		_fire_random_lasers()
		return
	...
	slot.finished.connect(_resolve_slot_results)
	_boss_banner("拉霸開獎中…")


func _resolve_slot_results(results: Array) -> void:
	# 組合規則：三同=Jackpot；兩同一異=強化版+單發；全異=三個單發依序
	var counts := {}
	for s in results:
		counts[str(s)] = int(counts.get(str(s), 0)) + 1
	var queue: Array = []
	if counts.size() == 1:
		queue.append({"sym": str(results[0]), "tier": 3})
	elif counts.size() == 2:
		for s in counts.keys():
			if int(counts[s]) == 2:
				queue.append({"sym": str(s), "tier": 2})
		for s in counts.keys():
			if int(counts[s]) == 1:
	...
	for entry in queue:
		...
		var t := get_tree().create_timer(delay, false, false, true)
		t.timeout.connect(func() -> void:
			if is_instance_valid(self) and health > 0.0:
				_fire_slot_attack(sym, tier)
		)
		delay += gap
	# 全部發動後解除開獎狀態（恢復移動/普攻/技能計時）
	var end_timer := get_tree().create_timer(delay + 0.4, false, false, true)
...
func _fire_slot_attack(sym: String, tier: int) -> void:
	match sym:
		"seven":
			match tier:
				1:
		...
		"bell":
			match tier:
				...
				2:
					_boss_banner("彈幕加倍！")
					_slot_bullet_ring(12, 0.0)
					var t2 := get_tree().create_timer(0.45, false, false, true)
					t2.timeout.connect(func() -> void:
						if is_instance_valid(self) and health > 0.0:
							_slot_bullet_ring(12, 15.0)
					)
				3:
					_boss_banner("彈幕地獄！！")
					for wave in range(3):
		...
		"joker":
			match tier:
				1:
					_boss_banner("召喚增援！")
					_slot_summon_basic(1)
				2:
					_boss_banner("大量增援！")
					_slot_summon_basic(2)
				3:
					_boss_banner("傾巢而出！！")
					for _i in range(6):
						_summon_one("retail")
					_summon_one("headhunter")
		"cherry":
			match tier:
				3:
					_boss_banner("櫻桃大獎！！！")
					_slot_cherry_jackpot()
				_:
					pass   # 空轉：不顯示技能名稱、不發動任何攻擊
		"skull":
			match tier:
				1:
					_boss_banner("狂暴！")
				...
				3:
					...
					_boss_enrage(8.0)
					for i in range(4):
						var tl := get_tree().create_timer(1.0 + float(i) * 1.0, false, false, true)
						tl.timeout.connect(func() -> void:
							if is_instance_valid(self) and health > 0.0:
								_cast_laser_at_angle(randf_range(0.0, TAU))
						)


func _boss_banner(text: String) -> void:
	var parent := get_parent()
	if parent != null and parent.has_method("_show_boss_skill_banner"):
		parent._show_boss_skill_banner(text)


func _cast_laser_at_angle(angle: float) -> void:
	var start := global_position
	var end := start + Vector2.RIGHT.rotated(angle) * 16.0 * TILE_SIZE
	_spawn_line_warning(start, end, 34.0, Color(1.0, 0.05, 0.05, 0.5), 1.0,
		func() -> void:
...
func _slot_lasers(count: int, spread_deg: float) -> void:
	for i in range(count):
		var ang: float
		if is_instance_valid(target):
			ang = (target.global_position - global_position).angle() \
				+ deg_to_rad(spread_deg) * (float(i) - float(count - 1) * 0.5)
		else:
			ang = randf_range(0.0, TAU)
		_cast_laser_at_angle(ang)


func _slot_laser_storm() -> void:
	# 777 Jackpot：6 道雷射繞場依序掃射（60 度間隔，0.3 秒錯開）
	var base := randf_range(0.0, TAU)
	for i in range(6):
		var t := get_tree().create_timer(0.3 * float(i), false, false, true)
		var ang := base + TAU * float(i) / 6.0
		t.timeout.connect(func() -> void:
			if is_instance_valid(self) and health > 0.0:
				_cast_laser_at_angle(ang)
		)
...
func _slot_shockwave(gap_count: int, gap_half_deg: float) -> void:
	var sw_script := load("res://AIgame_rougelike/scripts/boss_shockwave.gd")
	if sw_script == null:
		return
	var parent := get_parent()
	if parent == null:
		return
	var sw: Node2D = sw_script.new()
	sw.setup(target, 20.0 * TILE_SIZE, gap_count, gap_half_deg)
	parent.add_child(sw)
	sw.global_position = global_position
	_request_camera_shake(6.0)


func _slot_random_blasts(count: int) -> void:
	# 玩家周邊隨機落雷式爆破（各有 1.2 秒紅圈預警）
	if not is_instance_valid(target):
		return
	for _i in range(count):
		var pos: Vector2 = target.global_position + \
			Vector2.RIGHT.rotated(randf_range(0.0, TAU)) * randf_range(1.0, 7.0) * TILE_SIZE
		var blast_r := 2.2 * TILE_SIZE
		_spawn_circle_warning(pos, blast_r, Color(1.0, 0.1, 0.1, 0.5), 1.2,
			func() -> void:
				_spawn_expanding_ring(pos, blast_r * 0.2, blast_r, 0.3, Color(1.0, 0.5, 0.15, 0.85))
				if is_instance_valid(target) and pos.distance_to(target.global_position) <= blast_r:
					target.take_damage(1)
		)
...
func _slot_summon_basic(tier: int) -> void:
	...
	if enemy_id == "boss_final":
		...
		var retail_count := 6 if tier <= 1 else 9
		for _i in range(retail_count):
			_summon_one("retail")


func _summon_one(summon_id: String) -> void:
	var pos := global_position + Vector2(randf_range(-90.0, 90.0), randf_range(-90.0, 90.0))
	get_parent().call_deferred("_spawn_wave_enemy", summon_id, pos)


func _slot_cherry_jackpot() -> void:
	# 賭場超派彩：Boss 自損 5% 最大生命，並派彩晶片給玩家
	take_damage(max_health * 0.05)
	var parent := get_parent()
	if parent != null and parent.has_method("_add_total_chips"):
		parent._add_total_chips(2, "櫻桃大獎派彩：+%d 晶片")
	# 金幣噴發視覺
	if parent == null:
		return
	for _i in range(10):
		var coin := Polygon2D.new()
		coin.add_to_group("transient_effects")
		coin.z_index = 25
		var pts := PackedVector2Array()
		for j in range(8):
			pts.append(Vector2.RIGHT.rotated(TAU * float(j) / 8.0) * 7.0)
		coin.polygon = pts
		coin.color = Color(1.0, 0.85, 0.25, 1.0)
		parent.add_child(coin)
		coin.global_position = global_position
		var fly := coin.global_position + Vector2(randf_range(-160.0, 160.0), randf_range(-140.0, 40.0))
		var tw := create_tween()
		tw.set_pause_mode(Tween.TWEEN_PAUSE_STOP)
		tw.tween_property(coin, "global_position", fly, 0.55).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
		tw.parallel().tween_property(coin, "modulate:a", 0.0, 0.8)
...
func _boss_enrage(duration: float) -> void:
	if not _enrage_applied:
		_enrage_applied = true
		move_speed *= 1.4
		attacks_per_second *= 1.3
	_enrage_timer = maxf(_enrage_timer, duration)
	queue_redraw()
...
func _draw() -> void:
	...
	elif _enrage_timer > 0.0:
		tint = Color(1.0, 0.5, 0.45)   # 狂暴：泛紅發光
